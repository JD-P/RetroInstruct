...
async def main():
    ...
    pbar = tqdm(total=len(lorem_ipsum), desc="Evaluating lorem ipsum")
    while lorem_ipsum:
        batch = []
        while len(batch) < 64:
            try:
                passage = lorem_ipsum.pop()
            except IndexError:
                break
        ...
        scores = await evaluate_fn(rubric_score_fns, batch, port=5001)
