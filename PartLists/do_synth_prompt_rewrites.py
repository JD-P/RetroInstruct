...
def make_score_prompt_fn(template, suffix, response, _placeholder):
    return template.format(response=response) + suffix
...
if __name__ == "__main__":
    ...
    generate_fn = partial(generate_outputs, generator, batch_size=args.batch_size)
    evaluate_fn = partial(evaluate_outputs, evaluator)

    # Prefilter dictionary
    # Debian dictionary path
    with open("prompts/synth_prompt_seeds.txt") as infile:
        seeds = infile.read()
    synth_prompts = []
    # Pool so TQDM can give estimated completion time
    batch_indices = batched([i for i in range(1024)], 32)
    for _ in tqdm(batch_indices):
        candidate_prompts = generate_fn(seeds, 128, n=32)
        candidate_prompts = [candidate.split("\n")[0] for candidate in candidate_prompts]
        evaluation_prompt = prepare_rubric("prompts/synth_prompt_rewrites_rubric.txt")
        ...
        scores = score_fn([[candidate, ''] for candidate in candidate_prompts])
        for i, score in enumerate(scores):
            if score.item() > 10:
                synth_prompts.append(candidate_prompts[i])
                print(f"{candidate_prompts[i]}")
    with open("synth_prompts.json", "w") as outfile:
        json.dump(synth_prompts, outfile)
