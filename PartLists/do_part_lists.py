...
async def main():
    ...
    parts_prompt = """<s> [INST]Write me a list of 20 parts in a {WORD}, make sure to include enough items to uniquely identify it as a {WORD}. Do not use the word '{WORD}' in your description and answer only with a list of parts. Write 20 list items then stop.[/INST]"""
    ...
    for i, word_batch in enumerate(tqdm(word_batches)):
        prompts = [parts_prompt.format(WORD=word) for word in word_batch]
        waiting_gens = []
        for prompt in prompts:
            waiting_gens.append(asyncio.create_task(generate_fn(prompt, 512)))
        lists = []
        for waiting_gen in waiting_gens:
            await waiting_gen
            lists.append(waiting_gen.result().strip())
        for j in range(len(word_batch)):
            word_parts[word_batch[j]] = lists[j]
            print(f"{word_batch[j]}")
            print(lists[j])
        ...
        #        print(lists[i])
        if i % 100 == 0:
            with open("word_parts.json", "w") as outfile:
                json.dump(word_parts, outfile)
